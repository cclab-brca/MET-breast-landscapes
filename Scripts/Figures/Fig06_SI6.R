...
##   This R-script can be used to calculate Immunophenoscore (IPS) and generate Immunophenogram from "EXPRESSION.txt" and "IPS_genes.txt"
...
gene_expression<-read.table(".../Immunophenogram-master/autopsy-basedLog2_tpms_final.txt",row.names=1,header=TRUE, sep="\t", dec = ".",check.names=FALSE)
sample_names<-names(gene_expression)

## Read IPS genes and corresponding weights from tab-delimited text file "IPS_genes.txt"
IPSG<-read.table(.../Immunophenogram-master/IPS_genes.txt",header=TRUE, sep="\t", dec = ".",check.names=FALSE)
unique_ips_genes<-as.vector(unique(IPSG$NAME))
head(unique_ips_genes)
IPS<-NULL
MHC<-NULL
CP<-NULL
EC<-NULL
...
## Legend sample-wise (averaged) z-scores
  ...
  legendtheme <- theme(plot.margin = unit(c(2,0,2,0),"inch"), panel.spacing = unit(0,"null"), panel.grid.major = element_blank(),panel.grid.minor = element_blank(),panel.border = element_blank(),panel.background = element_blank(), axis.line = element_line(colour = "white"), axis.text=element_blank(), axis.ticks= element_blank(), axis.title.x=element_blank())
...
## Legend weighted z-scores
  ...
  pdf(file=".../Legend weighted z-scores.pdf", width=10, height=8)
  plot_c
  dev.off()
  
  pdf(file=".../Legend sample-wise z-scores.pdf", width=10, height=8)
  plot_b
  dev.off()
...
#write.table(DF,file="../IPS_autopsy.txt",row.names=FALSE, quote=FALSE,sep="\t")
