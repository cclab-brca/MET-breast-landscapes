...
## output directory
outdir <- '~/Documents/Caldas/Data/dataResults/treeomics/WES/'
...
cases <- c(288,'290exOvary',291,'298Brain','298main',308,315,323,328,330,'DET52','DET52main')
for (case in cases) {
  if (case=='290exOvary') {
    baseCase=290
  ...
  dat <- scan(paste0(datadir2,'/output_WES_selected/extractedTrees/',case,'.txt'),what='character')
  ...
  dat <- fread(paste0(datadir2,'/input_WES_selected/X',case,'_covCount.txt'),select=c('Chromosome','Position','Gene'),key = c('Chromosome','Position'))
  ...
  postFile <- list.files(path=paste0(datadir2,'/output_WES_selected/X',case,'/'),pattern='posterior.txt',full.names = TRUE)
  ...
  # edgeLengths$Filtered <- table(factor(edgeAttributes[!is.na(Keep),EdgeIdx],levels=1:ecount(eTree$tree)))
  # edgeLengths$FilteredNeoAntigens <- table(factor(edgeAttributes[!is.na(Keep)&neoAntigen==1,EdgeIdx],levels=1:ecount(eTree$tree)))
  ...
  height=8
  h.adjust=NULL
  pdf(paste0(outdir,'/',case,'_treeomics_treeOnly.pdf'),width = width,height = height)
  ...
  tikz(paste0(outdir,'/',case,'_treeomics_treeOnly.tex'),width = width,height = height)
  par(mar=c(1,4,0,3),xpd=TRUE)
  plotTree(tree = collapsed$tree,
           e.length = collapsed$e.length,
           clones=collapsed$labels,
  ...
  save(collapsed,file=(paste0(outdir,'/',case,'treeomics_tree.RData')))
  
  ###### Plot Tree with edge annotation #####
  pdf(paste0(outdir,'/',case,'_treeomics_tree_scaled.pdf'),width = 9+2*ecount(collapsed$tree),height = max(9,12*max(edgeLengths$All)/60))
  ...
  text(x=0,y=1.06,labels="All mutations",cex=0.75,adj=c(0,1),font=2)
  for (i in 1:n) {
  ...
  if (any(edgeLengths$AllNeoAntigens==0)) {
    collapsed <- collapseTree(tree = eTree$tree,
  ...
  if (baseCase%in%c(308,315)) {
    pdf(paste0(outdir,'/',case,'_treeomics_tree_scaled_NeoAntigens.pdf'),width = 9+ecount(collapsed$tree)/2, height= 10)
  } else {
    pdf(paste0(outdir,'/',case,'_treeomics_tree_scaled_NeoAntigens.pdf'),width = 9+ecount(collapsed$tree)/2, height= max(7,7*max(edgeLengths$AllNeoAntigens)/20))
  }
  par(mfrow=c(1,2),mar=c(3,4,3,0))
  plotTree(tree = collapsed$tree,
           e.length = collapsed$e.length,
